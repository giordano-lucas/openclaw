...
const base64UrlSecretKeyPattern = /^[A-Za-z0-9_-]{43}$/u;
const hexSecretKeyPattern = /^[a-f0-9]{64}$/u;
...
describe("scripts/lib/openclaw-test-state", () => {
  it("creates a sourceable env file and JSON description", async () => {
    ...
    try {
      ...
      expect(payload.env.OPENCLAW_AUTH_PROFILE_SECRET_KEY).toMatch(base64UrlSecretKeyPattern);
  ...
  it("renders a Docker-friendly shell snippet", async () => {
    ...
    try {
      ...
      expect(payload.secretKey).toMatch(hexSecretKeyPattern);
  ...
  it("keeps shell key generation independent of node", async () => {
    ...
    try {
      ...
      expect(shellProbe.stdout).toMatch(hexSecretKeyPattern);
      ...
      expect(functionProbe.stdout).toMatch(hexSecretKeyPattern);
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
    }
  });
